...
ch. 12 pg. 323 of the book.
    ...
    def _leftRotate(self, x):
        ...
        # Line 2-4 of pseudocode
        x.setRChild(y_left)
        if y_left is not None:
            y_left.setParent(x)

        # Line 5-10 of pseudocode
        if parent is None:
            self._root = y
        elif x.isLeftChild():
            parent.setLChild(y)
    ...
    def _insertFixup(self, z):
        """
        Fixes the tree post-insertion to comply with RBT properties
        """
        p = z.getParent()

        # Root node must be black from P2 (Case 0: Z is the root)
        while p is not None and p.isRed():
            gp = p.getParent()

            if gp is not None:
                if p.isLeftChild():
                    y = gp.getRChild()  # Uncle
                    if y is not None and y.isRed():
                        p.setBlack()
                        y.setBlack()
                        gp.setRed()
                        z = gp # But if the loop doesn't go z won't be assigned >:(
                        # Pseudocode 1-8
                    else:
                        if z.isRightChild():  # A TRIANGLE :O
                            self._leftRotate(p)
                        p.setBlack()
                        gp.setRed()
                        self._rightRotate(gp)
                else:
                    y = gp.getLChild()  # Uncle
                    if y is not None and y.isRed():
                        p.setBlack()
                        y.setBlack()
                        gp.setRed()
        ...
        # In this case, we make GP-P-Z a straight line:
        # ROTATE LEFT IF Z IS LEFT CHILD (SWAPS Z AND P) then recolor P
        # ROTATE RIGHT IF Z IS RIGHT CHILD (SWAPS Z AND P) then recolor P


        #Case 3: Z has a black uncle and GP-P-Z is a straight line (can come from case 2)
        # Objective: Make the tree more balanced
        # Recolor P, recolor GP, rotate grandpa
        # ROTATE LEFT IF Z IS THE RIGHT CHILD
        # ROTATE RIGHT IF Z IS THE LEFT CHILD
